...
function background_drag(event){
	let container_computed_style = getComputedStyle(background_container.value)
	let cursor_position = {
		x: event.pageX,
		y: event.pageY
	}
	...
	if(initial_position.value != null){
		...
		background_image.addEventListener('load',(event) => {
			convert_background_image_dimensions_to_pixels(event.target,background_container.value)
		})
		
		let container_box = background_container.value.getBoundingClientRect()
...
function move_background_image(cursor_position){
	if(cursor_is_not_at_screen_origin(cursor_position)){
		...
		const position = {
			x: parseFloat(container_computed_style.backgroundPosition.split('px',2).shift().trim()),
			y: parseFloat(container_computed_style.backgroundPosition.split('px',2).pop().trim())
		}

		const distance_x = (cursor_position.x - container_box.left) - initial_position.value.x
		const distance_y = (cursor_position.y - container_box.top) - initial_position.value.y
